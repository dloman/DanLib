  ...
  namespace detail
  {
    namespace sha256
    {
      ...
      constexpr sha256sum sha256tole(const sha256sum& sum)
      {
        return
        {{
          endianswap<uint32_t>(sum.h[0]),
          endianswap<uint32_t>(sum.h[1]),
          endianswap<uint32_t>(sum.h[2]),
          endianswap<uint32_t>(sum.h[3]),
          endianswap<uint32_t>(sum.h[4]),
          endianswap<uint32_t>(sum.h[5]),
          endianswap<uint32_t>(sum.h[6]),
          endianswap<uint32_t>(sum.h[7]),
        }};
      }
    }
  }
